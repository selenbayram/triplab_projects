...
### Load Seurat object
```{r}
Seu_smart <- readRDS("seu_smart_2.rds")
```
...
```{r}
VlnPlot(Seu_smart, features = c("SST", "LAMP5", "PVALB", "VIP", "GAD1", "SCN2A"))
```

```{r}
VlnPlot(Seu_smart, features = c("SST", "LAMP5", "PVALB", "VIP", "GAD1", "SCN2A"), group.by = "subclass_label")
```
...
```{r}
# metadata
meta_10x <- read.csv("/Users/selenbayram/triplab/KCNI_Summer_2023/SEA-AD_10x_meta_mini.csv", row.names = 1)
...
```{r}
Seu_smart@meta.data <- Seu_smart@meta.data %>% mutate(sex_cluster = paste(donor_sex_label, seurat_clusters, sep = "_"))
Idents(Seu_smart) <- "sex_cluster"
f_vs_m_c2 <- FindMarkers(Seu_smart, ident.1 = "M_2", ident.2 = "F_2", logfc.threshold = 2, min.pct = 0.5)
f_vs_m_c2
```
...
saveRDS(Seu_10x, file = "seu_10x.rds")
```
...
features_10x <- Seu_10x@meta.data %>% select(nCountRNA, nFeatureRNA, subclass_label) %>% mutate(method = "10x")
features_smart <- Seu_smart@meta.data %>% select(nCountRNA, nFeatureRNA, subclass_label) %>% mutate(method = "smart")
...
ggplot(features_combined, aes(x = method, y = log10(genes_detected), fill = method)) +
  geom_boxplot() +
  labs(x = "Method", y = "Number of detected genes") +
  ggtitle("Comparison of genes detected per sample between 10x and SMART-seq") +
  theme_classic() +
...
ggplot(features_combined, aes(x = nCountRNA, y = nFeatureRNA, color = method)) +
  geom_point(aes(color = subclass_label)) +
  geom_smooth(method = "lm") +
  facet_wrap(~method, scales = "free_y") +
  stat_cor(method = "spearmann") + 
  theme_classic()
```
